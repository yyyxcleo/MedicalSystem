    ...
    <el-row :gutter="20">
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
          ...
          <el-form-item label="状态" prop="eqStatus">
            <el-select
          ...
          <el-form-item label="工作区" prop="arId">
            <el-select
...
<!--          </el-form-item>-->
        ...
        <el-table v-loading="loading" :data="eqList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="设备序号" align="center"  v-if="columns[0].visible" >
            <template slot-scope="scope">
              {{ scope.$index + 1 }}
            </template>
          </el-table-column>
          <el-table-column label="设备名称" align="center" key="eqName" prop="eqName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
<!--          <el-table-column label="用户昵称" align="center" key="nickName" prop="nickName" v-if="columns[2].visible" :show-overflow-tooltip="true" />-->
          <el-table-column label="设备类型" align="center"  prop="eqType" v-if="columns[3].visible" :show-overflow-tooltip="true" >
            <template slot-scope="scope">
              <!-- 在列的 slot 中使用计算属性获取 eqType 对应的 eqtName -->
              {{ getEqtName(scope.row.eqType) }}
            </template>
          </el-table-column>
          <el-table-column label="所在区域" align="center" key="arId" prop="arId" v-if="columns[4].visible" width="120" >
            <template slot-scope="scope">
              <!-- 在列的 slot 中使用计算属性获取 eqType 对应的 eqtName -->
...
export default {
  ...
  methods: {
    ...
    filterNode(value, data) {
      ...
      return data.eqtName.indexOf(value) !== -1;
    },
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const userIds = row.eqId || this.ids;
      this.$modal.confirm('是否确认删除用户编号为"' + userIds + '"的数据项？').then(function() {
        return delEquipment(userIds);
      }).then(() => {
        this.getList();
